...
from scripts.process_events import realtime_process
from src.realtime.vq_tokenizer_rt import RealTimeVQTokenizer
from src.agent.model import AgentTransformer
from src.agent.build_sequences import (
    AgentWindowConfig,
    TokenConfig,
    TransformerConfig,
...
class RealTimeAgentRunner:
    def __init__(
        ...
        self.tokenizer = RealTimeVQTokenizer(
            vq_ckpt=vq_ckpt,
            scaler_path=str(PATHS.scalers / "scaler_norm2.pkl"),
            device=device,
        )
...
def main_rt():
    ...
    runner = RealTimeAgentRunner(
        pair=pair,
        vq_ckpt=str(PATHS.models / "vq_out" / "vqvae_best_1311.pt"),  # ajuste se mudar o caminho
        agent_ckpt=str(PATHS.checkpoints / "checkpoints_agent_stf" / "stf_best.pt"),
        device="cuda",
    )
