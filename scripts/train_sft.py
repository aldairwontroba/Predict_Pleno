...
def train_agent_stf(
    ...
    out_dir: str = str(PATHS.checkpoints / "checkpoints_agent_stf"),
    seed: int = 42,
):
...
def main():
    ...
    OUT_DIR = str(PATHS.checkpoints / "checkpoints_agent_stf")

    # checkpoint inicial do modelo pré-treinado (LM puro)
    # deixe como None para treinar SFT do zero,
    # ou coloque algo como "artifacts/checkpoints/checkpoints_agent_tiny/agent_best.pt"
    # INIT_CKPT = None
    INIT_CKPT = str(PATHS.checkpoints / "checkpoints_agent_tiny" / "agent_best.pt")
